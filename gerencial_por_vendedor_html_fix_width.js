...
const horaGeradaBR=(()=>{const d=new Date();const p=n=>String(n).padStart(2,"0");return `${p(d.getHours())}:${p(d.getMinutes())}:${p(d.getSeconds())}`;})();
let tmpCriado="";
let dbPath=fdb||fbk;
...
.top .left{display:flex;gap:5px;align-items:center;justify-content:space-between;align-content:space-around;min-width:0;flex:1 1 520px;flex-wrap:wrap;height:40px}
...
<div class="badge">Hora que foi gerado: ${escHtml(horaGeradaBR)}</div>
